...
class DeezerPlaylistIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        geoblocking_msg = self._html_search_regex(
            r'<p class="soon-txt">(.*?)</p>', webpage, 'geoblocking message', default=None
        )
        if geoblocking_msg is not None:
            raise ExtractorError(f'Deezer said: {geoblocking_msg}', expected=True)

        data_json = self._search_regex(
            (r'__DZR_APP_STATE__\s*=\s*({.+?})\s*</script>', r'naboo\.display\(\'[^\']+\',\s*(.*?)\);\n'),
            webpage,
            'data JSON',
        )
        ...
        playlist_thumbnail = self._search_regex(
            r'<img id="naboo_playlist_image".*?src="([^"]+)"', webpage, 'playlist thumbnail'
        )

        preview_pattern = self._search_regex(
            r"var SOUND_PREVIEW_GATEWAY\s*=\s*'([^']+)';", webpage, 'preview URL pattern', fatal=False
        )
        entries = []
        for s in data['SONGS']['data']:
            puid = s['MD5_ORIGIN']
            preview_video_url = (
                preview_pattern.replace('{0}', puid[0]).replace('{1}', puid).replace('{2}', s['MEDIA_VERSION'])
            )
            formats = [
                {
                    'format_id': 'preview',
            ...
            entries.append(
                {
                    ...
                    'title': '{} - {}'.format(artists, s['SNG_TITLE']),
                    'uploader': s['ART_NAME'],
