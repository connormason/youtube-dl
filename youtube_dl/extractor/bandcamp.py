...
class BandcampIE(InfoExtractor):
    ...
    def _extract_data_attr(self, webpage, video_id, attr='tralbum', fatal=True):
        return self._parse_json(
            self._html_search_regex(rf'data-{attr}=(["\'])({{.+?}})\1', webpage, attr + ' data', group=2),
            video_id,
            fatal=fatal,
        )
...
class BandcampAlbumIE(BandcampIE):
    ...
    def suitable(cls, url):
        return False if BandcampWeeklyIE.suitable(url) or BandcampIE.suitable(url) else super().suitable(url)
