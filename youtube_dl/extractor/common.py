...
class InfoExtractor:
    ...
    * optionally re-define the _real_initialize() method.
    ...
    def _og_regexes(prop):
        ...
        property_re = (r'(?:name|property)=(?:\'og[:-]{prop}\'|"og[:-]{prop}"|\s*og[:-]{prop}\b)'.format(prop=re.escape(prop)))
        template = r'<meta[^>]+?%s[^>]+?%s'
        return [
                ...
                for f in m3u8_formats:
                    if f['vcodec'] != 'none':
                        for protocol in ('http', 'https'):
                            ...
                            del http_f['manifest_url']
                            http_url = re.sub(
                                REPL_REGEX, protocol + rf'://{http_host}/\g<1>{qualities[i]}\3', f['url'])
                            http_f.update({
    ...
    def _yes_playlist(self, playlist_id, video_id, *args, **kwargs):
        ...
        self.to_screen(
            ...
            if noplaylist else
            'Downloading {}{} - add --no-playlist to download just the {}{}'.format(
