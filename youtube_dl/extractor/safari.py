...
class SafariBaseIE(InfoExtractor):
    ...
    def _login(self):
        ...
        _, urlh = self._download_webpage_handle(
            'https://learning.oreilly.com/accounts/login-check/', None, 'Downloading login page'
        )
...
class SafariIE(SafariBaseIE):
    ...
    def _real_extract(self, url):
        ...
        if reference_id:
            ...
            if not reference_id:
                reference_id = self._search_regex(
                    r'data-reference-id=(["\'])(?P<id>(?:(?!\1).)+)\1', webpage, 'kaltura reference id', group='id'
                )
        ...
        return self.url_result(
            update_url_query('https://cdnapisec.kaltura.com/html5/html5lib/v2.37.1/mwEmbedFrame.php', query), 'Kaltura'
        )
...
class SafariApiIE(SafariBaseIE):
    ...
    def _real_extract(self, url):
        ...
        part = self._download_json(
            url, '{}/{}'.format(mobj.group('course_id'), mobj.group('part')), 'Downloading part JSON'
        )
...
class SafariCourseIE(SafariBaseIE):
    ...
    def suitable(cls, url):
        return False if SafariIE.suitable(url) or SafariApiIE.suitable(url) else super().suitable(url)
