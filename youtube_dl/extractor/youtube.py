...
class YoutubeBaseInfoExtractor(InfoExtractor):
    ...
    def _login(self):
        ...
        login_page = self._download_webpage(
            self._LOGIN_URL, None, note='Downloading login page', errnote='unable to fetch login page', fatal=False
        )
        ...
        lookup_results = req(self._LOOKUP_URL, lookup_req, 'Looking up account info', 'Unable to look up account info')
        ...
        challenge_results = req(self._CHALLENGE_URL, challenge_req, 'Logging in', 'Unable to log in')
        ...
        if login_res:
            ...
            warn(
                'Unable to login: {}'.format('Invalid password')
        ...
        if login_challenge:
            ...
            if challenge_str == 'TWO_STEP_VERIFICATION':
                ...
                tfa_results = req(self._TFA_URL.format(tl), tfa_req, 'Submitting TFA code', 'Unable to submit TFA code')
                ...
                if tfa_res:
                    ...
                    warn(
                        'Unable to finish TFA: {}'.format('Invalid TFA code')
                        if tfa_msg == 'INCORRECT_ANSWER_ENTERED'
                        else tfa_msg
                    )
                    return False

                check_cookie_url = try_get(tfa_results, lambda x: x[0][-1][2], compat_str)
            else:
                CHALLENGES = {
                ...
                challenge = CHALLENGES.get(challenge_str, f'{self.IE_NAME} returned error {challenge_str}.')
        ...
        check_cookie_results = self._download_webpage(check_cookie_url, None, 'Checking cookie', fatal=False)
    ...
    def _generate_sapisidhash_header(self, origin='https://www.youtube.com'):
        ...
        if self._SAPISID is None:
            ...
            sapisid_cookie = dict_get(yt_cookies, ('__Secure-3PAPISID', 'SAPISID'))
            if sapisid_cookie and sapisid_cookie.value:
                ...
                if not yt_cookies.get('SAPISID'):
                    ...
                    self._set_cookie('.youtube.com', 'SAPISID', self._SAPISID, secure=True, expire_time=time_now + 3600)
            else:
                self._SAPISID = False
        if not self._SAPISID:
            return None
        # SAPISIDHASH algorithm from https://stackoverflow.com/a/32065323
        sapisidhash = hashlib.sha1(f'{time_now} {self._SAPISID} {origin}'.encode()).hexdigest()
        return f'SAPISIDHASH {time_now}_{sapisidhash}'

    def _call_api(self, ep, query, video_id, fatal=True, headers=None, note='Downloading API JSON'):
        ...
        return self._download_json(
            f'https://www.youtube.com/youtubei/v1/{ep}',
            video_id=video_id,
            note=note,
    ...
    def _extract_yt_initial_data(self, video_id, webpage):
        return self._parse_json(
            self._search_regex(
                (rf'{self._YT_INITIAL_DATA_RE}\s*{self._YT_INITIAL_BOUNDARY_RE}', self._YT_INITIAL_DATA_RE),
                webpage,
                'yt initial data',
            ),
            video_id,
    ...
    def _extract_video(self, renderer):
        ...
        title = try_get(
            ...
            (
                ...
                lambda x: x['headline']['simpleText'],
            ),
            compat_str,
        )
        description = try_get(renderer, lambda x: x['descriptionSnippet']['runs'][0]['text'], compat_str)
        duration = parse_duration(try_get(renderer, lambda x: x['lengthText']['simpleText'], compat_str))
        view_count_text = try_get(renderer, lambda x: x['viewCountText']['simpleText'], compat_str) or ''
        view_count = str_to_int(
            self._search_regex(r'^([\d,]+)', re.sub(r'\s', '', view_count_text), 'view count', default=None)
        )
        uploader = try_get(
            renderer,
            (lambda x: x['ownerText']['runs'][0]['text'], lambda x: x['shortBylineText']['runs'][0]['text']),
            compat_str,
        )
        return {
            '_type': 'url',
    ...
    def _extract_thumbnails(data, *path_list, **kw_final_key):
        ...
        return traverse_obj(
            data,
            (
                (tuple((*variadic(path), final_key, Ellipsis) for path in path_list or [()])),
                {
                    'url': (
                        'url',
                        T(url_or_none),
    ...
    def _search_results(self, query, params):
        ...
        for page_num in itertools.count(1):
            ...
            slr_contents = try_get(
                search,
                (
                    lambda x: x['contents']['twoColumnSearchResultsRenderer']['primaryContents']['sectionListRenderer'][
                        'contents'
                    ],
            ...
            for slr_content in slr_contents:
                isr_contents = try_get(slr_content, lambda x: x['itemSectionRenderer']['contents'], list)
                if not isr_contents:
                    continue
    ...
    def _extract_channel_id(self, webpage, videodetails={}, metadata={}, renderers=[]):
        ...
        if any((videodetails, metadata, renderers)):
            channel_id = (
                ...
                or traverse_obj(
                    renderers,
                    [*self._owner_endpoints_path(), 'navigationEndpoint', 'browseEndpoint', 'browseId'],
                    get_all=False,
                )
            )
        return channel_id or self._html_search_meta('channelId', webpage, 'channel id', default=None)

    def _extract_author_var(self, webpage, var_name, videodetails={}, metadata={}, renderers=[]):
        result = None
        paths = {
        ...
        if any((videodetails, metadata, renderers)):
            result = (
                ...
                or traverse_obj(renderers, self._owner_endpoints_path() + paths[var_name][3], get_all=False)
            )
        return result or traverse_obj(
            extract_attributes(
                self._search_regex(
                    rf"""(?s)(<link\b[^>]+\bitemprop\s*=\s*("|'){re.escape(var_name)}\2[^>]*>)""",
    ...
    def _extract_uploader_id(self, uploader_url):
        return self._search_regex(
            r'/(?:(?:channel|user)/|(?=@))([^/?&#]+)', uploader_url or '', 'uploader id', default=None
        )
...
class YoutubeIE(YoutubeBaseInfoExtractor):
    ...
    def _extract_player_url(self, *ytcfgs, **kw_webpage):
        ...
        if webpage:
            player_url = self._search_regex(
                r'"(?:PLAYER_JS_URL|jsUrl)"\s*:\s*"([^"]+)"', webpage or '', 'player URL', fatal=False
            )
            if player_url:
                ytcfgs = (*ytcfgs, {'PLAYER_JS_URL': player_url})
        return traverse_obj(
            ytcfgs,
            (Ellipsis, 'PLAYER_JS_URL'),
            (Ellipsis, 'WEB_PLAYER_CONTEXT_CONFIGS', Ellipsis, 'jsUrl'),
            get_all=False,
            expected_type=lambda u: urljoin('https://www.youtube.com', u),
        )

    def _download_player_url(self, video_id, fatal=False):
        res = self._download_webpage(
            'https://www.youtube.com/iframe_api', note='Downloading iframe API JS', video_id=video_id, fatal=fatal
        )
    ...
    def _extract_player_info(self, player_url):
        try:
            return self._search_regex(self._PLAYER_INFO_RE, player_url, 'player info', group='id')
        except ExtractorError as e:
            raise ExtractorError(f'Cannot identify player {player_url!r}', cause=e)

    def _player_js_cache_key(self, player_url, extra_id=None, _cache={}):
        if player_url not in _cache:
            player_id = self._extract_player_info(player_url)
            player_path = remove_start(compat_urllib_parse.urlparse(player_url).path, f'/s/player/{player_id}/')
            variant = next((k for k, v in self._PLAYER_JS_VARIANT_MAP if v == player_path), None)
            if not variant:
                variant = next(
                    (
                        k
                        for k, v in self._PLAYER_JS_VARIANT_MAP
    ...
    def _extract_signature_function(self, video_id, player_url, example_sig):
        ...
        cache_spec, code = self._load_player_data_from_cache('sigfuncs', player_url, extra_id=extra_id), None
        ...
        if not cache_spec:
            ...
            if code:
                ...
                self._store_player_data_to_cache('sigfuncs', player_url, cache_spec, extra_id=extra_id)
            else:
                self.report_warning(f'Failed to compute signature function {player_url}-{extra_id}')
    ...
    def _print_sig_code(self, func, example_sig):
        ...
        signature_id_tuple = '({})'.format(', '.join(compat_str(len(p)) for p in example_sig.split('.')))
    ...
    def _extract_n_function_name(self, jscode):
        ...
        def generic_n_function_search(func_name=None):
            return self._search_regex(
                r"""(?xs)
                    ...
                    (?P<name>{})\s*=\s*function\((?!\d)[a-zA-Z\d_$]+\)
                    \s*\{{(?:(?!}};).)+?(?:
                        ["']enhanced_except_ |
                        return\s*(?P<q>"|')[a-zA-Z\d-]+_w8_(?P=q)\s*\+\s*[\w$]+
                    )
                """.format(func_name or r'(?!\d)[a-zA-Z\d_$]+'),
                jscode,
        ...
        if not func_name:
            ...
            for m in re.finditer(
                ...
                fn = self._search_regex(
                    r'[;,]\s*(function\s+)?({})(?(1)|\s*=\s*function)\s*\((?!\d)[\w$]+\)\s*\{}(?!\s*return\s)'.format(
                        re.escape(m.group('nfunc')), '{'
                    ),
                    jscode,
                    'Initial JS player n function name (2)',
                    group=2,
                    default=None,
                )
                if fn:
        ...
        return self._search_json(
            rf'(?<![\w-])var\s(?:(?:(?!,).)+,|\s)*?{re.escape(func_name)}\s*=',
            jscode,
    ...
    def _mark_watched(self, video_id, player_response):
        playback_url = url_or_none(
            try_get(player_response, lambda x: x['playbackTracking']['videostatsPlaybackUrl']['baseUrl'])
        )
        if not playback_url:
            return
        ...
        video_length = '{}'.format(float((qs.get('len') or ['1.5'])[0]) - 1)
    ...
    def _extract_urls(webpage):
        ...
        entries.extend(list(map(unescapeHTML, re.findall(r'class="lazyYT" data-youtube-id="([^"]+)"', webpage))))
    ...
    def _extract_chapters_from_json(self, data, video_id, duration):
        chapters_list = try_get(
            data,
            lambda x: x['playerOverlays']['playerOverlayRenderer']['decoratedPlayerBarRenderer'][
        ...
        def chapter_time(chapter):
            return float_or_none(
                try_get(chapter, lambda x: x['chapterRenderer']['timeRangeStartMillis'], int), scale=1000
            )
        ...
        for next_num, chapter in enumerate(chapters_list, start=1):
            ...
            if start_time is None:
                continue
            end_time = chapter_time(chapters_list[next_num]) if next_num < len(chapters_list) else duration
            if end_time is None:
                continue
            title = try_get(chapter, lambda x: x['chapterRenderer']['title']['simpleText'], compat_str)
            chapters.append(
                {
    ...
    def _extract_yt_initial_variable(self, webpage, regex, video_id, name):
        return self._parse_json(
            self._search_regex((rf'{regex}\s*{self._YT_INITIAL_BOUNDARY_RE}', regex), webpage, name, default='{}'),
            video_id,
            fatal=False,
        )
    ...
    def _real_extract(self, url):
        ...
        webpage = self._download_webpage(webpage_url + '&bpctr=9999999999&has_verified=1', video_id, fatal=False)

        player_response = None
        player_url = None
        if webpage:
            player_response = self._extract_yt_initial_variable(
                webpage, self._YT_INITIAL_PLAYER_RESPONSE_RE, video_id, 'initial player response'
            )
        ...
        if True or not player_response:
            ...
            client_names = traverse_obj(
                self._INNERTUBE_CLIENTS, (T(dict.items), lambda _, k_v: not k_v[1].get('REQUIRE_PO_TOKEN'), 0)
            )[:1]
            if 'web' not in client_names:
                # webpage links won't download: ignore links and playability
                player_response = filter_dict(
                    player_response or {}, lambda k, _: k not in ('streamingData', 'playabilityStatus')
                )
            ...
            for client in traverse_obj(self._INNERTUBE_CLIENTS, (client_names, T(dict))):
                ...
                api_player_response = self._call_api(
                    ...
                    headers=api_headers,
                    note=join_nonempty(
                        'Downloading', traverse_obj(query, ('context', 'client', 'clientName')), 'API JSON', delim=' '
                    ),
                )
                ...
                if len(hls) == 2 and not hls[0] and hls[1]:
                    ...
                    video_details = merge_dicts(
                        *traverse_obj((player_response, api_player_response), (Ellipsis, 'videoDetails', T(dict)))
                    )
                    player_response.update(filter_dict(api_player_response or {}, cndn=lambda k, _: k != 'captions'))
        ...
        if is_agegated(playability_status) and int_or_none(self._downloader.params.get('age_limit'), default=18) >= 18:
        ...
        search_meta = (lambda x: self._html_search_meta(x, webpage, default=None)) if webpage else lambda _: None

        video_details = player_response.get('videoDetails') or {}
        microformat = try_get(player_response, lambda x: x['microformat']['playerMicroformatRenderer'], dict) or {}
        video_title = (
                        ...
                        entries.append(
                            {
                                ...
                                'ie_key': 'Youtube',
                                'url': smuggle_url(
                                    base_url + 'watch?v=' + feed_data['id'][0], {'force_singlefeed': True}
                                ),
                                'title': title,
                            }
                        )
            ...
            return LazyList(
                {
                    'url': update_url_query(
                        f['url'],
                        {
                            'range': '{}-{}'.format(range_start, min(range_start + CHUNK_SIZE - 1, f['filesize'])),
                        },
                    )
                }
        ...
        for fmt in streaming_formats:
            ...
            if not fmt_url:
                ...
                try:
                    ...
                    self.report_warning(
                        'Signature extraction failed: Some formats may be missing', video_id=video_id, only_once=True
                    )
            ...
            language_preference = (
                ...
                if 'descriptive' in (traverse_obj(audio_track, ('displayName', T(lower))) or '')
                else -1
            )
            name = traverse_obj(fmt, ('qualityLabel', T(txt_or_none))) or quality.replace('audio_quality_', '')
            dct = {
                ...
                'language': join_nonempty(
                    audio_track.get('id', '').split('.')[0], 'desc' if language_preference < -1 else ''
                )
                or None,
            ...
            mime_mobj = re.match(r'((?:[^/]+)/(?:[^;]+))(?:;\s*codecs="([^"]+)")?', fmt.get('mimeType') or '')
            if mime_mobj:
        ...
        def process_manifest_format(f, proto, client_name, itag, all_formats=False):
            ...
            if itag:
                f['format_id'] = f'{itag}-{proto}' if all_formats or any(p != proto for p, _ in itags[itag]) else itag
            ...
            f['quality'] = q(traverse_obj(f, ('format_id', T(lambda s: itag_qualities[s.split('-')[0]])), default=-1))
        ...
        if hls_manifest_url:
            for f in self._extract_m3u8_formats(
                hls_manifest_url, video_id, 'mp4', entry_protocol='m3u8_native', live=is_live, fatal=False
            ):
        ...
        if self._downloader.params.get('youtube_include_dash_manifest', True):
            ...
            if dash_manifest_url:
                for f in self._extract_mpd_formats(dash_manifest_url, video_id, fatal=False):
                    if process_manifest_format(f, 'dash', None, f['format_id']):
                        f['filesize'] = traverse_obj(
                            f,
                            (
                                ('fragment_base_url', 'url'),
                                T(lambda u: self._search_regex(r'/clen/(\d+)', u, 'file size', default=None)),
                                T(int_or_none),
                            ),
                            get_all=False,
                        )
        ...
        if formats:
            ...
            pemr = try_get(playability_status, lambda x: x['errorScreen']['playerErrorMessageRenderer'], dict) or {}
        ...
        for container in (video_details, microformat):
            for thumbnail in try_get(container, lambda x: x['thumbnail']['thumbnails'], list) or []:
        ...
        channel_id = self._extract_channel_id(webpage, videodetails=video_details, metadata=microformat)
        ...
        for f in formats:
            ...
            if try_call(lambda x: float(x.pop('_duration_ms')) / duration < 500, args=(f,)):
                self.report_warning(f'{video_id}: Some possibly damaged formats will be deprioritized', only_once=True)
        ...
        uploader = self._extract_author_var(webpage, 'name', videodetails=video_details, metadata=microformat)
        ...
        info = {
            ...
            'upload_date': unified_strdate(microformat.get('uploadDate') or search_meta('uploadDate')),
            'uploader': uploader,
            'channel_id': channel_id,
            'duration': duration,
            'view_count': int_or_none(
                video_details.get('viewCount') or microformat.get('viewCount') or search_meta('interactionCount')
            ),
        ...
        pctr = traverse_obj(
            (player_response, api_player_response), (Ellipsis, 'captions', 'playerCaptionsTracklistRenderer', T(dict))
        )
        if pctr:
            ...
            def process_subtitles():
                ...
                for caption_track in traverse_obj(
                    pctr, (Ellipsis, 'captionTracks', lambda _, v: (v.get('baseUrl') and v.get('languageCode')))
                ):
                    ...
                    if caption_track.get('kind') != 'asr':
                        ...
                        continue
                    automatic_captions = {}
                    process_language(automatic_captions, base_url, lang_code, {})
                    for translation_language in traverse_obj(
                        pctr, (Ellipsis, 'translationLanguages', lambda _, v: v.get('languageCode'))
                    ):
                ...
                info.update(
                    {
                        ...
                        or ', '.join(a.strip() for a in mobj.group('artist').split('·')),
        ...
        if not initial_data:
            initial_data = self._call_api('next', {'videoId': video_id}, video_id, fatal=False)

        if initial_data:
            chapters = self._extract_chapters_from_json(initial_data, video_id, duration)
            if not chapters:
                for engagment_pannel in initial_data.get('engagementPanels') or []:
                    contents = try_get(
                        engagment_pannel,
                        lambda x: x['engagementPanelSectionListRenderer']['content']['macroMarkersListRenderer'][
                            'contents'
                        ],
                        list,
                    )
                    ...
                    for next_num, content in enumerate(contents, start=1):
                        ...
                        end_time = (
                            chapter_time(try_get(contents, lambda x: x[next_num]['macroMarkersListItemRenderer']))
            ...
            for content in contents:
                ...
                if vpir:
                    ...
                    for tlb in (
                        try_get(vpir, lambda x: x['videoActions']['menuRenderer']['topLevelButtons'], list) or []
                    ):
                        tbr = (
                            traverse_obj(
                                tlb,
                        ...
                        for getter, regex in [
                            ...
                            if label:
                                ...
                                if mobj:
                                    ...
                                    break
                    sbr_tooltip = try_get(vpir, lambda x: x['sentimentBar']['sentimentBarRenderer']['tooltip'])
                    if sbr_tooltip:
                        ...
                        info['like_count'] = traverse_obj(
                            ...
                            (
                                ...
                                'buttonViewModel',
                                (('title', ('accessibilityText', T(lambda s: s.split()), Ellipsis))),
                                T(parse_count),
                            ),
                            get_all=False,
                        )
                ...
                if vsir:
                    rows = (
                        try_get(vsir, lambda x: x['metadataRowContainer']['metadataRowContainerRenderer']['rows'], list)
                        or []
                    )
...
class YoutubeTabIE(YoutubeBaseInfoExtractor):
    ...
    @classmethod
    def suitable(cls, url):
        return not YoutubeIE.suitable(url) and super().suitable(url)
    ...
    @staticmethod
    def _get_text(r, k):
        return traverse_obj(r, (k, 'runs', 0, 'text'), (k, 'simpleText'), expected_type=txt_or_none)
    ...
    def _grid_entries(self, grid_renderer):
        for item in traverse_obj(grid_renderer, ('items', Ellipsis, T(dict))):
            ...
            if channel_id:
                title = self._get_text(renderer, 'title')
                yield self.url_result(
                    f'https://www.youtube.com/channel/{channel_id}', ie=YoutubeTabIE.ie_key(), video_title=title
                )
                continue
            ...
            if ep_url:
                for ie in (YoutubeTabIE, YoutubePlaylistIE, YoutubeIE):
                    if ie.suitable(ep_url):
                        yield self.url_result(ep_url, ie=ie.ie_key(), video_id=ie._match_id(ep_url), video_title=title)
                        break
    ...
    def _shelf_entries(self, shelf_renderer, skip_channels=False):
        ep = try_get(
            shelf_renderer, lambda x: x['endpoint']['commandMetadata']['webCommandMetadata']['url'], compat_str
        )
    ...
    def _extract_lockup_view_model(self, view_model):
        ...
        return merge_dicts(
            ...
            {
                'title': traverse_obj(
                    view_model, ('metadata', 'lockupMetadataViewModel', 'title', 'content', T(compat_str))
                ),
    ...
    def _extract_shorts_lockup_view_model(self, view_model):
        ...
        if not content_id:
            return
        return merge_dicts(
            self.url_result(content_id, ie=YoutubeIE.ie_key(), video_id=content_id),
            {
                'title': traverse_obj(view_model, ('overlayMetadata', 'primaryText', 'content', T(compat_str))),
                'thumbnails': self._extract_thumbnails(view_model, 'thumbnail', final_key='sources'),
            },
        )
    ...
    def _post_thread_entries(self, post_thread_renderer):
        post_renderer = try_get(post_thread_renderer, lambda x: x['post']['backstagePostRenderer'], dict)
        if not post_renderer:
            return
        # video attachment
        video_renderer = try_get(post_renderer, lambda x: x['backstageAttachment']['videoRenderer'], dict)
        video_id = None
        if video_renderer:
        ...
        for run in runs:
            if not isinstance(run, dict):
                continue
            ep_url = try_get(run, lambda x: x['navigationEndpoint']['urlEndpoint']['url'], compat_str)
            if not ep_url:
                continue
    ...
    def _rich_grid_entries(self, contents):
        for content in traverse_obj(contents, (Ellipsis, 'richItemRenderer', 'content'), expected_type=dict):
            video_renderer = traverse_obj(content, 'videoRenderer', 'reelItemRenderer', expected_type=dict)
            if video_renderer:
                entry = self._video_entry(video_renderer)
                if entry:
                    yield entry
            ...
            if shorts_lockup_view_model:
                ...
                if entry:
                    yield entry
            # playlist
            renderer = traverse_obj(content, 'playlistRenderer', expected_type=dict) or {}
            title = self._get_text(renderer, 'title')
            playlist_id = renderer.get('playlistId')
            if playlist_id:
                yield self.url_result(
    ...
    def _extract_next_continuation_data(renderer):
        next_continuation = try_get(renderer, lambda x: x['continuations'][0]['nextContinuationData'], dict)
    ...
    def _extract_continuation(cls, renderer):
        ...
        for command in traverse_obj(
            ...
            (
                ...
                (('commandExecutorCommand', 'commands', Ellipsis), None),
                T(dict),
            ),
        ):
    ...
    def _entries(self, tab, item_id, webpage):
        ...
        if slr_renderer:
            ...
            for slr_content in slr_contents:
                ...
                for isr_content in isr_contents:
                    ...
                    if renderer:
                        for from_ in self._rich_grid_entries(traverse_obj(renderer, ('contents', Ellipsis, T(dict)))):
                            yield from_
                        continuation = self._extract_continuation(renderer)
                        continue
            ...
            if not rich_grid_renderer:
                return
            for from_ in self._rich_grid_entries(traverse_obj(rich_grid_renderer, ('contents', Ellipsis, T(dict)))):
                yield from_

            continuation = self._extract_continuation(rich_grid_renderer)

        ytcfg = self._extract_ytcfg(item_id, webpage)
        client_version = try_get(ytcfg, lambda x: x['INNERTUBE_CLIENT_VERSION'], compat_str) or '2.20210407.08.00'
        ...
        for page_num in itertools.count(1):
            ...
            while count <= retries:
                try:
                    ...
                    response = self._download_json(
                        ...
                        None,
                        'Downloading page %d%s' % (page_num, ' (retry #%d)' % count if count else ''),
                        query={
                            # 'key': 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8',
                            'prettyPrint': 'false',
                        },
            ...
            visitor_data = try_get(response, lambda x: x['responseContext']['visitorData'], compat_str) or visitor_data

            continuation_contents = try_get(response, lambda x: x['continuationContents'], dict)
            ...
            if continuation_items:
                ...
                if renderer:
                    ...
                    continue
                renderer = continuation_item.get('playlistVideoRenderer') or continuation_item.get(
                    'itemSectionRenderer'
                )
                if renderer:
    ...
    def _extract_uploader(self, metadata, data):
        ...
        uploader['uploader'] = self._extract_author_var('', 'name', renderers=renderers) or self._extract_author_var(
    ...
    def _extract_alert(cls, data):
        ...
        for alert in traverse_obj(data, ('alerts', Ellipsis), expected_type=dict):
            alert_text = traverse_obj(alert, (None, lambda x: x['alertRenderer']['text']), get_all=False)
            if not alert_text:
                continue
    ...
    def _extract_from_tabs(self, item_id, webpage, data, tabs):
        ...
        renderer = traverse_obj(data, ('metadata', 'channelMetadataRenderer'), expected_type=dict) or {}
        playlist_id = item_id
        ...
        if renderer:
            ...
            title = join_nonempty(
                channel_title or item_id, tab_title, txt_or_none(selected_tab.get('expandedText')), delim=' - '
            )
            ...
            title = traverse_obj(renderer, 'title', ('hashtag', 'simpleText'), expected_type=txt_or_none)
    ...
    def _extract_from_playlist(self, item_id, url, data, playlist):
        title = traverse_obj((playlist, data), (0, 'title'), (1, 'titleText', 'simpleText'), expected_type=txt_or_none)
        ...
        playlist_url = urljoin(
            url,
            traverse_obj(
                playlist, ('endpoint', 'commandMetadata', 'webCommandMetadata', 'url'), expected_type=url_or_none
            ),
        )
        if playlist_url and playlist_url != url:
            return self.url_result(playlist_url, ie=YoutubeTabIE.ie_key(), video_id=playlist_id, video_title=title)
        return self.playlist_result(self._playlist_entries(playlist), playlist_id=playlist_id, playlist_title=title)
    ...
    def _extract_identity_token(self, ytcfg, webpage):
        if ytcfg:
            ...
            if token:
                return token
        return self._search_regex(r'\bID_TOKEN["\']\s*:\s*["\'](.+?)["\']', webpage, 'identity token', default=None)
    ...
    def _real_extract(self, url):
        ...
        tabs = try_get(data, lambda x: x['contents']['twoColumnBrowseResultsRenderer']['tabs'], list)
        if tabs:
            return self._extract_from_tabs(item_id, webpage, data, tabs)
        playlist = try_get(data, lambda x: x['contents']['twoColumnWatchNextResults']['playlist']['playlist'], dict)
        if playlist:
            return self._extract_from_playlist(item_id, url, data, playlist)
        # Fallback to video extraction if no playlist alike page is recognized.
        # First check for the current video then try the v attribute of URL query.
        video_id = (
            try_get(data, lambda x: x['currentVideoEndpoint']['watchEndpoint']['videoId'], compat_str) or video_id
        )
        if video_id:
            return self.url_result(video_id, ie=YoutubeIE.ie_key(), video_id=video_id)
...
class YoutubePlaylistIE(InfoExtractor):
    ...
    _VALID_URL = rf"""(?x)(?:
                        ...
                        (?P<id>{YoutubeBaseInfoExtractor._PLAYLIST_ID_RE})
                     )"""
    ...
    def _real_extract(self, url):
        ...
        if not qs:
            qs = {'list': playlist_id}
        return self.url_result(
            update_url_query('https://www.youtube.com/playlist', qs), ie=YoutubeTabIE.ie_key(), video_id=playlist_id
        )
...
class YoutubeYtUserIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        return self.url_result(f'https://www.youtube.com/user/{user_id}', ie=YoutubeTabIE.ie_key(), video_id=user_id)
...
class YoutubeFavouritesIE(YoutubeBaseInfoExtractor):
    ...
    def _real_extract(self, url):
        return self.url_result('https://www.youtube.com/playlist?list=LL', ie=YoutubeTabIE.ie_key())
...
class YoutubeSearchIE(SearchInfoExtractor, YoutubeBaseInfoExtractor):
    ...
    def _get_n_results(self, query, n):
        ...
        entries = itertools.islice(
            self._search_results(query, self._SEARCH_PARAMS), 0, None if n == float('inf') else n
        )
...
class YoutubeFeedsInfoExtractor(YoutubeTabIE):
    ...
    def _real_extract(self, url):
        return self.url_result(f'https://www.youtube.com/feed/{self._FEED_NAME}', ie=YoutubeTabIE.ie_key())
...
class YoutubeWatchLaterIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        return self.url_result('https://www.youtube.com/playlist?list=WL', ie=YoutubeTabIE.ie_key())
...
class YoutubeTruncatedIDIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        video_id = self._match_id(url)
        raise ExtractorError(f'Incomplete YouTube ID {video_id}. URL {url} looks truncated.', expected=True)
